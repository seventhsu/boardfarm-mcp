#!/usr/bin/env python3
"""Full acquire-flash-debug-release cycle."""
import asyncio, sys
sys.path.insert(0, 'src')
from mcp.server.fastmcp import FastMCP
from mcp_boardfarm.server import create_server, app_lifespan, list_boards, reserve_board_with_timeout, release_board, flash_firmware, start_debug_session, stop_debug_session, read_registers
...
async def test():
    ...
    async with app_lifespan(mcp) as state:
        ...
        if "✓" in d:
            ...
            print(f"   Registers read: {len(regs)} chars")
